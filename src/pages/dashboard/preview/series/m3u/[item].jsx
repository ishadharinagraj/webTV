...
import { addToFavs, getFavourites, getParticluarTimeline, getValue, removeFromFavs } from '@/firebase/functions';
...
const MoviePreview = () => {
    ...
    useEffect(() => {
        if (item && user) {
            ...
            checkFav(user?.loginType ==="m3u" ? user?.id : user.dbAddress)
            setMovie(null);
            setMovieCast(null);
    ...
    useEffect(() => {
        if (movie && currentKeys && user && streamData.movies && user?.loginType ==="m3u") {
            getMovieTimeline()
            getMovieCast();
            getMoviesByCategory();
    ...
    const getMovie = async () => {
        ...
        const endpoints = {
            getId: `/search/movie?api_key=${process.env.NEXT_PUBLIC_TMBD_API_KEY}&query=`,
            getMovie: `/movie`
        }
        ...
        try {
            const response = await axios.get(process.env.NEXT_PUBLIC_TMDB_BASE + endpoints.getId + removeBrackets());
            ...
            if (currentYearMovie) {
                ...
                const movieInfo = await axios.get(process.env.NEXT_PUBLIC_TMDB_BASE + endpoints.getMovie + `/${tmbdbId}?api_key=${process.env.NEXT_PUBLIC_TMBD_API_KEY}`);
    ...
    const getMovieTimeline = async () => {
        try {
            const response = await getParticluarTimeline(concatUrl(streamUrl), 'Series', user?.id);
            if (response.val()) {
                // const { timeline, duration } = response.val();
                const { timeline, duration } = response.val()?.episode;
                const watched = (timeline / duration) * 100;
                setWatchedProgress(watched);
                setCurrentTimeline(watched < 100 ? response.val() ? response.val()?.episode?.timeline : null : 0);
    ...
    const getMovieCast = async () => {
        try {
            const response = await axios.get(process.env.NEXT_PUBLIC_CAST_URL + movie.id + '/credits?api_key=' + process.env.NEXT_PUBLIC_TMBD_API_KEY);
    ...
    const handleFavourites = async () => {
        ...
        if (addedToFav) {
            try {
                await removeFromFavs(concatUrl(streamUrl), "Series", finalAddress);
                alert.toggle({
            ...
            try {
                await addToFavs(concatUrl(streamUrl), "Series", finalAddress);
                alert.toggle({
    ...
    const checkFav = async (address) => {
        try {
            const res = await getFavourites('Series',address);
             const keys = res.val() ? Object.keys(res.val()) : [];
            setAddedToFav(keys.includes(concatUrl(streamUrl)))
        } catch (error) {
            console.log(error);
        }
    }
    ...
    const handleClose = () => {
        ...
        getMovieTimeline()
        setWatchedProgress(0)    
        navigate.push('/dashboard?view=series')
    }
                        ...
                        <div className="item">
                            ...
                            <div className="info">
                                ...
                                {/* <p className="text">{movie?.overview}</p> */}
                                 <p id="info-wrapper" className="text">
                                  {movie?.overview?.length > limit ? (
        <div>
          <Typography className="overview-text">
            {expanded
              ? <> {movie.overview} <p className="Show-more"  size="small"  sx={{ textTransform: "none", mt: 1 }}  onClick={() => setExpanded(!expanded)}>Show less</p> </>
              :<>{ movie.overview.slice(0, limit) + "..."}<p   className="Show-more"  size="small"  sx={{ textTransform: "none", mt: 1 }}  onClick={() => setExpanded(!expanded)} >Show more</p></>}
          </Typography>
        </div>
      ) : (
        <Typography className="overview-text">{movie.overview}</Typography>
      )}
</p>
            ...
            <Player
                type={"series-m3u"}
                timeline={currentTimeline ? currentTimeline : watchedProgress}
                id={concatUrl(streamUrl)}
                close={handleClose}
                src={streamUrl}
                loginType={user.loginType}

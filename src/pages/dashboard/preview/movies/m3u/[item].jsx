...
const MoviePreview = () => {
    ...
    // }, [])
    useEffect(()=>{console.log("inside movies peview")},[])
    useEffect(() => {
        if (user && streamUrl) {
            checkFav(user.id)
        }
    ...
    const getMovieTimeline = async () => {
        try {
            const response = await getParticluarTimeline(concatUrl(streamUrl), 'Movie', user?.id);
            if (response.val()) {
    ...
    const handleFavourites = async () => {
        ...
        if (addedToFav) {
            try {
                await removeFromFavs(concatUrl(streamUrl), "Movie", user?.id);
                alert.toggle({
            ...
            try {
                await addToFavs(concatUrl(streamUrl), "Movie", user?.id);
                alert.toggle({
    ...
    const handleClose = () => {
        ...
        if (state === "play" || yearNotDefined) {
            navigate.push('/dashboard?view=movies')
        }
                    ...
                    <section className="mainBanner previewMainBanner">
                        <div className="item">
                            <div className="thumb">
                                ...
                                <span className="effectGrad"></span><img src={movie?.poster_path ? 'https://image.tmdb.org/t/p/original' + movie?.poster_path : "/placeholder.jpg"} /></div>
                            <div className="info">
                                ...
                                <p className="text"><b>Genre:</b> {movie?.genres.map((genre, index) => genre?.name)}  <b>Directed By:</b> {movieCrew?.[0]?.name}</p>

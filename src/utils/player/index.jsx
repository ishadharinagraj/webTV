    ...
    if (time > 10) {
      if (type !== 'm3u') {
        if (type === "series") {
          ...
          try {
            await saveWatchedContent.series(id, user.id, {
              season: '',
              episode: '',
              timeline: time,
              thumbnail: '',
                ...
                </div>
                {
                  ...
                  <div className='speed-menu' >
                    <div className='dropdown-container'>
                      {
                        speedOptions.map(option => <p key={option} className={speed === option ? "current-selected" : ""} onClick={() => handleSpeed(option)}>{option}x</p>)
                      }
                    </div>
                  </div>
                }
